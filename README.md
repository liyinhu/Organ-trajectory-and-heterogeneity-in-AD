...
This repository contains the analysis code and illustrative input files for a plasma proteomics study of Alzheimer’s disease (AD). The project uses plasma protein profiles to derive organ-resolved risk indices and characterize their trajectories and heterogeneity during the years preceding AD diagnosis.
...
The example files are synthetic and intended to demonstrate input schemas. They are not study data and are too small for reliable scientific inference. Intermediate files under directories such as `diff-results/` are generated by preceding scripts and are therefore not duplicated as example inputs.
...
R package requirements are listed in `docs/r-packages.txt`. Several packages must be installed from Bioconductor.

## Running the examples

Run scripts from the matching example directory because most scripts use relative input paths.
...
For functional analysis, `organ_pathway_scores.csv` is a direct example input for `02-ssgsea-linear-model.R`. In a real sequential run, replace it with the score matrix generated by `01-organ-ssgsea.R`.
...
The code is released under the MIT License. Confirm that study data and third-party annotations can be redistributed before committing them to a public repository.
